...
def image_to_dice(input_path, output_svg="dice_art.svg", output_png="dice_preview.png",
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            pad = cell * 0.07
            w = h = cell - 2*pad
            # Couleur du dé basée sur la luminosité
            m = means[j, i]
            v = 1.0 - (m/255.0)
            r = int(240 - 100*v)
            g = int(240 - 120*v)
            b = int(245 - 120*v)
            fill = f"rgb({r},{g},{b})"
            svg.append(svg_rect(x+pad, y+pad, w, h, rx=cell*0.12, ry=cell*0.12,
                                fill=fill, stroke="#222"))
            pip_r = max(1.8, cell*0.06)
...
if __name__ == "__main__":
    ...
    image_to_dice("1710150656226.jpeg", "dice_art.svg", "dice_preview.png", cell=24)
